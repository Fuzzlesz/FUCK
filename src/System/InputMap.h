...
namespace Input::Keymap
{
	...
	inline std::pair<ImGuiKey, bool> ToImGuiKey(GAMEPAD_DIRECTX a_key)
	{
		switch (a_key) {
	...
	inline std::pair<ImGuiKey, bool> ToImGuiKey(GAMEPAD_ORBIS a_key)
	{
		switch (a_key) {
	...
	inline const char* GetKeyName(std::uint32_t a_key)
	{
